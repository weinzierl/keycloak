...
//    }
        ...
        try {
            ...
            if (realmUrl.equals(issuer)) {
                ...
                if (isExpired(exp)) {
                    ...
                    eventBuilder.event(EventType.INTROSPECT_TOKEN_ERROR).error("Expired token");
            ...
            ObjectNode tokenMetadata = JsonSerialization.createObjectNode();
            tokenMetadata.put("active", false);
            eventBuilder.detail("Token introspection failed", e.getMessage());
            eventBuilder.event(EventType.INTROSPECT_TOKEN_ERROR).error("Token introspection failed");
            try {
                return Response.ok(JsonSerialization.writeValueAsBytes(tokenMetadata)).type(MediaType.APPLICATION_JSON_TYPE).build();
    ...
    protected Response introspectKeycloak (String token, EventBuilder eventBuilder) {
        ...
        try {
            ...
            eventBuilder.event(EventType.INTROSPECT_TOKEN_ERROR).error(e.getMessage());
            throw new RuntimeException("Error creating token introspection response.", e);
        }
    }
    ...
    protected AccessToken verifyAccessToken(String token, EventBuilder eventBuilder) {
        ...
        return transformAccessToken(accessToken, tokenManager.checkTokenValidForIntrospection(session, realm, accessToken, false), eventBuilder);
    }
...
//                return Response.ok(cachedToken).type(MediaType.APPLICATION_JSON_TYPE).build();
            ...
            eventBuilder.event(EventType.INTROSPECT_TOKEN_ERROR).error("Could not perform remote introspection");
            ObjectNode tokenMetadata = JsonSerialization.createObjectNode();
            tokenMetadata.put("active", false);
            return Response.ok(JsonSerialization.writeValueAsBytes(tokenMetadata)).type(MediaType.APPLICATION_JSON_TYPE).build();
        } catch (Exception e) {
