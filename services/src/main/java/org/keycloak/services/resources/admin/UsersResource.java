    ...
    public Response createUser(final UserRepresentation rep) {
        ...
        try {
            ...
            if (rep.getGroups() != null) {
                ...
                for (UserGroupMembershipRepresentation member :rep.getGroups()) {
                    GroupModel group = KeycloakModelUtils.findGroupByPath(realm, member.getGroup().getPath());
                    if (group != null) {
                        auth.groups().requireManageMembers(group);
                        auth.groups().requireManageMembership(group);
                    } else {
                        return ErrorResponse.error(String.format("Group %s not found", member.getGroup().getPath()), Response.Status.BAD_REQUEST);
                    }
                }
            } else {
